...
# Export S3 credentials generated by the garage-init container
eval $(docker run --rm -v sendrec_garage-keys:/keys:ro alpine cat /keys/env)

# Frontend dev server with hot reload (port 5173, proxies API to 8080)
make dev-web

# Go server (in another terminal, also needs the same eval above)
make run
```
